...
新生成的配置默认启用 `PRIVACY_MODE=true`，公开 CN 流量和 WebRTC STUN 都经代理，避免检测页面同时观察到国内直连与代理出口；局域网地址仍保持直连，原有 Apple/Spotify 规则保持不变。DNS 在 Stash 使用 `follow-rule`、在 Mihomo 使用 `respect-rules` 经代理查询，并保留独立 bootstrap 解析器防止递归依赖。设置 `PRIVACY_MODE=false` 可恢复传统国内直连分流。

普通流量默认使用 `🛟 自动故障切换`：Reality 正常时行为不变，连接失败时按 Reality → CDN → Hysteria2 → AnyTLS 顺序切换。AI 域名则使用单独的 `🤖 AI 隐私出口`，固定到服务端 IPv4 的 Reality（CDN-only 时为 CDN）并在故障时失败关闭，避免切换节点后 IP 地址族漂移。
